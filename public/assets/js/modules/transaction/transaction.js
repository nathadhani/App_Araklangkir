        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if (data !== undefined) {
                    if (data !== '[]' && data.length > 0){
                        var d = JSON.parse(data)[0];
                        var sisa_stock = (d.ending_stock === null ? 0 : Number(d.ending_stock));
                        if(sisa_stock > 0){
                            $("#qty_stock").val(sisa_stock);
                            if(xtr_id == 2){ // Trx OUT
                                $("#qty").val(sisa_stock);
                            }
                        }
...
$("#btn-add-detail").on('click', function (e) {
    ...
    if ( $("#tr_id").val() === null || $("#tr_id").val() === '' ){
        ...
        if(xtr_id == 2){ // Trx OUT
            var xsisastock = $("#qty_stock").val();
            if(xsisastock > 0){                
                var qty_input = Number($("#qty").val());
                if( xsisastock < qty_input ){
                    alertify.alert('Stok kurang, hanya tersedia ' + xsisastock + '!');
                    $("#qty").val(xsisastock);
                    return false;
                }
                if( xsisastock >= qty_input ){
            ...
            success: function(data) {
                if(data.length > 0){
                    try {
                        ...
                        $('#btn-reset-detail').prop('disabled', true);
                    } catch (e) {
                        alertify.error("error");
                    }                        
                }                        
